...
@section('content')
<div class="max-w-4xl mx-auto" x-data="editUnitForm()" x-init="init()">
    <div class="form-container rounded-3xl shadow-2xl overflow-hidden fade-in">
        <!-- Enhanced Header -->
        <div class="form-header p-8 text-white">
            <div class="relative z-10 flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold mb-2">Edit Unit Usaha</h1>
                    <p class="text-blue-100 text-lg">{{ $unitUsaha->nama_unit_usaha }}</p>
                </div>
        ...
        <div class="p-8 pb-0">
            <div class="info-card bounce-in">
                <div class="flex items-center space-x-4">
                    <div class="w-16 h-16 bg-gradient-to-r from-blue-500 to-blue-600 rounded-2xl flex items-center justify-center text-white shadow-lg">
                        <svg class="w-8 h-8" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M3 4a1 1 0 011-1h12a1 1 0 011 1v2a1 1 0 01-1 1H4a1 1 0 01-1-1V4zM3 10a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H4a1 1 0 01-1-1v-6zM14 9a1 1 0 00-1 1v6a1 1 0 001 1h2a1 1 0 001-1v-6a1 1 0 00-1-1h-2z"/>
                        </svg>
                    </div>
                    <div class="flex-1">
                        <h3 class="text-lg font-bold text-gray-800 mb-1">Informasi Unit Usaha</h3>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                            <div>
                                <span class="text-gray-500">Jumlah Barang:</span>
                                <span class="font-semibold text-blue-600 ml-2">{{ $unitUsaha->barangs()->count() }} item</span>
                            </div>
                            <div>
                                <span class="text-gray-500">Dibuat:</span>
                                <span class="font-semibold text-gray-700 ml-2">{{ $unitUsaha->created_at->format('d M Y') }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500">Terakhir Update:</span>
                                <span class="font-semibold text-gray-700 ml-2">{{ $unitUsaha->updated_at->diffForHumans() }}</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="p-8 pt-0 relative z-10">
            <form action="{{ route('pengurus.unit-usaha.update', $unitUsaha->id) }}" method="POST" class="space-y-8" @submit="handleSubmit">
                @csrf
                @method('PUT')
                ...
                <div class="flex flex-col sm:flex-row justify-end space-y-4 sm:space-y-0 sm:space-x-6 pt-8 bounce-in" style="animation-delay: 0.3s">
                    ...
                    <button type="submit" 
                            ...
                            class="update-btn px-8 py-4 rounded-2xl font-semibold transition-all duration-300 flex items-center justify-center space-x-3 min-w-[200px]">
                        <template x-if="!isSubmitting">
                            <div class="flex items-center space-x-3">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/>
                                </svg>
                                <span>Update Unit Usaha</span>
                            </div>
                        </template>
                        <template x-if="isSubmitting">
                            <div class="flex items-center space-x-3">
...
function editUnitForm() {
    return {
        formData: {
            nama_unit_usaha: '{{ old('nama_unit_usaha', $unitUsaha->nama_unit_usaha) }}',
            deskripsi: '{{ old('deskripsi', $unitUsaha->deskripsi) }}'
        },
        isSubmitting: false,
        originalData: {
            nama_unit_usaha: '{{ $unitUsaha->nama_unit_usaha }}',
            deskripsi: '{{ $unitUsaha->deskripsi }}'
        },
        ...
        checkForChanges() {
            const hasChanges = this.formData.nama_unit_usaha !== this.originalData.nama_unit_usaha ||
                              this.formData.deskripsi !== this.originalData.deskripsi;
            
            if (hasChanges) {
                // Add visual indicator for unsaved changes
                document.title = '• Edit Unit Usaha - Koperasi';
            } else {
                document.title = 'Edit Unit Usaha - Koperasi';
            }
        },
        ...
        handleSubmit(event) {
            ...
            if (!hasChanges) {
                this.showNotification('Tidak ada perubahan yang perlu disimpan', 'warning');
                if (event) event.preventDefault();
                return false;
            }
            
            this.isSubmitting = true;
            this.showNotification('Mengupdate unit usaha...', 'info');
